...
jQuery(document).ready(function($){
	...
	/*} else {
		...
		$.each(hooks, function(key, value) { 
			...
			if ($.cookie("of_current_opt") == '#of-option-'+ value) {
				...
				var clicked_group = $('#of-nav li.' + value).children('ul').children('li:first').addClass('current').children('a').attr('href');
				$(clicked_group).fadeIn(500);
			}
	...
	$('#of-nav li ul.sub-menu li a').click(function(evt){
		
		$(this).parent().parent().parent().addClass('current');
	})
	
	$('#of-nav li.has-children a').click(function(evt){
	...
	$('#expand_options').click(function(){
		if(flip == 0){
			...
			$('#of_container #content').css({'margin-left':'0'})
			...
			$('#of_container #content').css({'margin-left':'194px'})
	...
	/**	Tipsy @since v1.3 */
	if (jQuery().tipsy) {
		$('.typography-size, .typography-height,.typography-md-custom, .typography-face, .typography-style, .of-typography-color').tipsy({
			fade: true,
			gravity: 's',
			opacity: 0.7,
		});
