    ...
    class dbt_certificate_managerPage extends Page
    {
        ...
        protected function CreateGridSearchControl(Grid $grid)
        {
            ...
            $grid->SearchControl = new SimpleSearch('dbt_certificate_managerssearch', $this->dataset,
                array('certificate_managerID', 'certificate_number', 'company_name', 'certificate_status', 'cert_standard', 'issue_date', 'expiry_date', 'address', 'country', 'company_locations', 'city', 'secondary_location', 'scope', 'notes'),
                array($this->RenderText('Certificate ManagerID'), $this->RenderText('Certificate Number'), $this->RenderText('Company Name'), $this->RenderText('Certificate Status'), $this->RenderText('Cert Standard'), $this->RenderText('Issue Date'), $this->RenderText('Expiry Date'), $this->RenderText('Address'), $this->RenderText('Country'), $this->RenderText('Company Locations'), $this->RenderText('City'), $this->RenderText('Secondary Location'), $this->RenderText('Scope'), $this->RenderText('Notes')),
                array(
        ...
        protected function CreateGridAdvancedSearchControl(Grid $grid)
        {
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('company_locations', $this->RenderText('Company Locations')));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('issue_date_edit');
            ...
            //
            $editor = new TextEdit('expiry_date_edit');
            ...
            //
            // View column for company_locations field
            //
            $column = new TextViewColumn('company_locations', 'Company Locations', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('company_locations_handler');
            
            /* <inline edit column> */
            //
            // Edit column for company_locations field
            //
            $editor = new TextAreaEdit('company_locations_edit', 50, 8);
            $editColumn = new CustomEditColumn('Company Locations', 'company_locations', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        protected function CreateGrid()
        {
            ...
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'company_locations_handler', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for city field
            //
