...
class sidebar_generator {
			...
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.onError = function() { alert('<?php __('Ajax error. Cannot add sidebar','framework')?>' )};
				  	mysack.runAJAX();
					return true;
				}
	...
	function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die("alert(".__("Sidebar already exists, please use a different name.","framework").")");
		}
	...
	function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("<?php _e('Are you sure you want to remove','framework')?> " + name + "?\n<?php _e('This will remove any widgets you have assigned to this sidebar.','framework')?>");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php _e('Sidebar Name:','framework')?>","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php _e('Sidebar Generator:','framework')?></h2>
			<p><?php _e('The sidebar name is for your use only. It will not be visible to any of your visitors. <br>
				A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.','framework')?>
			</p>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php _e('Add a Sidebar','framework')?>">+ <?php _e('Add Sidebar','framework')?></a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="Remove this sidebar"><?php _e('remove','framework')?></a></td>
				</tr>
				<?php
						$cnt++;
					}
	...
	*/
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox">
			<div class="handlediv" title="<?php _e('Click to toggle','framework')?>"><br /></div><h3 class='hndle'><span><?php _e('Sidebars','framework')?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p><?php _e('Select the sidebar you wish to display on this page/post<br><br>
						<strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars. ','framework')?>
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo $i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php _e('WP Default Sidebar','framework')?></option>
							<?php
