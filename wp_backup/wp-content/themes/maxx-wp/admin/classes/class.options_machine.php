...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ( $headings as $k => $v ) {
			$token = 'of-option-' . preg_replace( '/[^a-zA-Z0-9\s]/', '', strtolower( trim( str_replace( ' ', '', $v['name'] ) ) ) );
...
//			
			...
			switch ( $value['type'] ) {
				...
				//tab 
//				case 'tab':
//					if($value['tab_title']) $output .= '<dt class="tab-heading">'. $value['tab_title'] .'</dt>'."\n";
//					$output .= '<dd class="tab-content">'."\n";
//
//				 break;
				...
				case 'multicheck': 			
					...
					foreach ($value['options'] as $key => $option) {
						...
						$output .= '<div class="m-inline-multicheck"><input type="checkbox" class="checkbox of-input" name="'.$value['id'].'['.$key.']'.'" id="'. $of_key_string .'" value="1" '. checked($multi_stored[$key], 1, false) .' /><label class="multicheck" for="'. $of_key_string .'">'. $option .'</label></div>';								
					}			 
				break;
				...
				case 'typography':
					...
					/* Letter Spacing */
					if(isset($typography_stored['letterspacing'])) {
					
						$output .= '<div class="select_wrapper typography-md-custom" original-title="Letter Spacing">';
						$output .= '<select class="of-typography of-typography-md-custom select" name="'.$value['id'].'[letterspacing]" id="'. $value['id'].'_letterspacing">';
							for ($i = 0; $i < 31; $i++){ 
								$test = $i.'px';
								$output .= '<option value="'. $i .'px" ' . selected($typography_stored['letterspacing'], $test, false) . '>'. $i .'px</option>'; 
								}
					...
					/* Margin bottom */
					if(isset($typography_stored['marginbottom'])) {
					
						$output .= '<div class="select_wrapper typography-md-custom" original-title="Margin Bottom">';
						$output .= '<select class="of-typography of-typography-md-custom select" name="'.$value['id'].'[marginbottom]" id="'. $value['id'].'_marginbottom">';
							for ($i = 10; $i < 55; $i+=5){ 
								$test = $i.'px';
								$output .= '<option value="'. $i .'px" ' . selected($typography_stored['marginbottom'], $test, false) . '>'. $i .'px</option>'; 
								}
				...
				case "subheading":
					...
					$output .= '<div class="group" id="'. $jquery_click_hook  .'"><h2 class="subheading-title">'.$value['name'].'</h2>'."\n";
				break;
